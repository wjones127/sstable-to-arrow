...
#include <arrow/table.h>                        // For PromoteTableSchema...
...
struct SSTableRecordBatchReader : public arrow::RecordBatchReader
{
    SSTableRecordBatchReader(std::map<int, std::shared_ptr<sstable_t>> sstables,
        ...
        : sstable_i(0), output_schema(output_schema), pool(pool), sstable_ids(sstables.size()),
          sstables(sstables.size())
    {
        int i = 0;
        for (auto &entry : sstables)
        {
            sstable_ids[i] = entry.first;
            sstables[i] = entry.second;
            i++;
        }
    }
    ...
    arrow::Status ReadNext(std::shared_ptr<arrow::RecordBatch> *batch)
    {
        ...
        std::cout << "\n\n========== Reading SSTable #" << sstable_id << " ==========\n";

        // Load as a table
        ARROW_RETURN_NOT_OK(sstable->init());
